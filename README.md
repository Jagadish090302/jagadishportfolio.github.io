...
### Admin edit lock
Clicking **EDIT MODE** now opens an admin passcode dialog. The included lock is a client-side convenience for a static site; it should not be treated as server-grade authentication. To change the passcode, replace `ADMIN_PASSCODE_HASH` in `script.js` with the SHA-256 hash of your own passcode.
